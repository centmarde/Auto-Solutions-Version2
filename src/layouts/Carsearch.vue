        ...
        <v-form @submit.prevent="submitForm">
          <v-row>
            <v-col cols="12" md="6" class="pa-10">
              ...
              <div class="mb-3">
                ...
                <select id="brand" class="form-select" v-model="selectedMake" @change="filterModels">
              ...
              <div class="mb-3" v-if="selectedMake">
                ...
                <select id="model" class="form-select" v-model="selectedModel" @change="showCarImage">
                  ...
                  <option v-for="model in filteredModels" :key="model.Model" :value="model.Model">
                    {{ model.Model }}
                  </option>
                </select>
              </div>
              ...
              <div class="mb-3">
                ...
                <textarea class="form-control" id="description" rows="3"
                  :placeholder="selectedCar ? selectedCar.Description : 'Car Description'"></textarea>
              </div>
            </v-col>
              ...
              <transition name="fade" mode="out-in">
                <v-card v-if="selectedCar" key="card" elevation="10" class="mt-3">
                  <v-card-text class="text-center">
                    ...
                    <v-img :src="selectedCar.ImageURL" :alt="selectedCar.Model" class="my-3" width="100%"></v-img>
                  </v-card-text>
                </v-card>
              </transition>
            </v-col>
          ...
          <v-row>
            <v-col class="d-flex justify-center mt-3">
              <!-- Submit Button -->
              <router-link to="/login" class="tr" exact>
                <v-btn type="submit" color="primary" class="btnn">
                  <span>Show Matches {{ $count }}</span>
                  <v-icon right>
                    mdi-arrow-right
                  </v-icon>
                </v-btn>
