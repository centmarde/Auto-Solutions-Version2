        ...
        <v-card elevation="8" class="pa-4 hover-card shine-effect">
          ...
          <v-row no-gutters>
            <v-col cols="12">
              ...
              <v-card-text>
                Secure your investment and stay ahead of market trends! Insert
                your car here to effortlessly store your vehicle in your
                personal garage and keep track of its market value based on
                real-time sales data. Adding your car today ensures you're
                always informed about its worth, whether you're planning to
                sell, upgrade, or hold onto it for future gains.
              </v-card-text>
      ...
      try {
        ...
        await Promise.all(
          this.cars.map(async (car) => {
            ...
            const randomNamesResponse = await fetch(
              "https://randomuser.me/api/?results=" + marketValueData.length
            );
            const randomNamesData = await randomNamesResponse.json();

            const randomNames = randomNamesData.results.map(
