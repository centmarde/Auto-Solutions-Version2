  ...
  <v-container class="my-5">
    ...
    <v-dialog v-model="dialog" max-width="400px">
      <v-card>
        ...
        <v-card-text>
          Your car has been successfully submitted and is now pending review by
          our team. Check the Inquires page for updates.
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    checkPrice() {
      if (isNaN(this.car.price)) {
        const userConfirmed = confirm(
          " Price must be a numeric value. Example: 5000"
        );
        if (userConfirmed) {
          this.car.price = ""; // Clears the price field
        }
      }
    },
